...
def main() -> int:
    ...
    if not voice_id:
        print("Copy .env.example to .env and add ELEVENLABS_VOICE_ID")
        return 1
    ...
    if what in ("all", "ask"):
        for item in spec.get("pauses", []) + spec.get("resumes", []):
            dest = OUT / item["file"]
            print(f"ask {item['file']}")
            tts(item["text"], dest, api_key, voice_id, model_id)
